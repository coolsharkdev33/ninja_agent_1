...
# goup_9 robustness features
_RETRYABLE_HTTP_CODES = {429, 500, 502, 503, 504}  # includes rate-limit (429)
_MAX_LINE_LENGTH = 500  # For output sanitization [FROM goup_9]
_HEAVY_CMD_RE = re.compile(r"\b(pip|npm|yarn|pnpm|cargo|mvn|gradle|make|apt|yum|pacman|brew)\b")
_HEAVY_CMD_TIMEOUT = 30  # seconds for pip/npm/cargo/make operations
...
def chat_completion(
    ...
    [ENHANCED WITH goup_9: handles 429 rate-limit with exponential backoff]
    """
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            # goup_9: Check if error code is retryable (includes 429 for rate-limit)
            retryable = e.code in _RETRYABLE_HTTP_CODES
            if retryable and attempt < max_retries:
                last_error = e
                # goup_9: exponential backoff with cap at 15 seconds
...
def run_command(command: str, cwd: Path, timeout: int = DEFAULT_COMMAND_TIMEOUT) -> CommandResult:
    ...
    # goup_9: Detect heavy commands (pip/npm/cargo/make/etc) and extend timeout
    effective_timeout = timeout
    if _HEAVY_CMD_RE.search(command):
        effective_timeout = max(timeout, _HEAVY_CMD_TIMEOUT)
    ...
    try:
        ...
        # goup_9: Sanitize output (remove null bytes, control chars, truncate long lines)
        ...
        return CommandResult(
            ...
            stderr=_truncate(stderr, MAX_OBSERVATION_CHARS),
        ...
        # goup_9: Sanitize timeout output
        ...
        return CommandResult(
            ...
            exit_code=124,
            stdout=_truncate(stdout, MAX_OBSERVATION_CHARS),
            stderr=_truncate(stderr + f"\nCommand timed out after {effective_timeout}s.", MAX_OBSERVATION_CHARS),
            duration_sec=time.time() - start,
            timed_out=True,
        )
...
def format_observation(result: CommandResult) -> str:
    """Format command result for model observation.
    [ENHANCED WITH goup_9: prioritizes stderr on error, shows timeout/blocked flags]
    """
    parts = [
        "COMMAND:",
        result.command,
    ...
    # goup_9: Show timeout flag
    if result.timed_out:
        parts.extend(["", "TIMED_OUT: true"])

    # goup_9: Show blocked flag
    if result.blocked:
        parts.extend(["", "BLOCKED: true"])

    # goup_9: Prioritize stderr when exit_code != 0 (errors often in stderr)
...
_THINK_RE = re.compile(r"<think>.*?</think>", re.IGNORECASE | re.DOTALL)  # goup_9: for stripping think blocks


def _strip_think_blocks(text: str) -> str:
    """Remove <think>...</think> blocks before parsing for commands/finals.
    [FROM goup_9]"""
...
def extract_final(model_text: str) -> Optional[str]:
    # goup_9: Strip think blocks before searching for final
    cleaned_text = _strip_think_blocks(model_text)
    match = FINAL_RE.search(cleaned_text)
    if not match:
        return None
